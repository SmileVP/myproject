...
public class UserController {
    ...
    //? make the response type dynamic
    public ResponseEntity<?> login(@RequestBody LoginDto loginDto){
        String token = userService.verifyLogin(loginDto);
        if(token!=null){
            //token response is dto so that we are mentioning to return the response as json instead of string
            // why json response because angular team will use this url and they will extract the json to the url
